package cards.fullscreen_card_mode;
import org.testng.annotations.Test;

public class FullscreenCardModeTest {

    //TC ID TRE060 Checking the possibility to edit a card's Title by add and change text in a fullscreen mode.
    @Test
    public static void titleTest(){}

    //TC ID TRE061 Under title "in list" dropdown menu test.
    @Test
    public static void inListTest(){}

    //TC ID TRE062 Checking the possibility to edit a card's Description by add and change text in a fullscreen mode.
    @Test
    public static void descriptionTest(){}

    //TC ID TRE063 Attachments from PC section testing.
    @Test
    public static void attachmentsFromPCTest(){}

    //TC ID TRE064 Attachments from Trello section testing.
    @Test
    public static void attachmentsFromTrelloTest(){}

    //TC ID TRE065 Attachments from Google Drive section testing.
    @Test
    public static void attachmentsFromGoogleDriveTest(){}

    //TC ID TRE066 Attachments from Dropbox section testing.
    @Test
    public static void attachmentsFromDropboxTest(){}

    //TC ID TRE067 Attachments from Box section testing.
    @Test
    public static void attachmentsFromBoxTest(){}

    //TC ID TRE068 Attachments from Onedrive section testing.
    @Test
    public static void attachmentsFromOnedriveTest(){}

    //TC ID TRE069 Attachments by a link testing.
    @Test
    public static void attachmentsByLinkTest(){}

    //TC ID TRE070 Write a comment module testing.
    @Test
    public static void writeCommentTest(){}

    //TC ID TRE071 Add to card: Members
    @Test
    public static void addMembersToCardTest(){}

    //TC ID TRE072 Test Labels and their names are displaying.
    @Test
    public static void labelsTest(){}

    //TC ID TRE073 Check the behaviour of checklist in a fullscreen card.
    @Test
    public static void checklistTest(){}

    //TC ID TRE074 Check the behaviour of card when date is created, edited and completed.
    @Test
    public static void datesTest(){}

    //TC ID TRE075 Checking the Custom Fields Premium is required.
    @Test
    public static void customFieldsPremiumRequireTest(){}

    //TC ID TRE076 Show/Hide details button testing.
    @Test
    public static void showHideButtonTest(){}

    //TC ID TRE077 Cards fullscreen opening test.
    @Test
    public static void cardsFullscreenOpeningTest(){}

    //TC ID TRE078 Power-Ups functionality test.
    @Test
    public static void powerUpsFunctionalityTest(){}

    //TC ID TRE079 Checking editing functions of the Actions menu.
    @Test
    public static void actionsMenuTest(){}

    // TC ID TRE 080 Check the board could be exported by the Share button to suggest formats and the links are valid.
    @Test
    public static void shareButtonTest(){}
}
